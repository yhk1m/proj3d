...
## 2026-09-22 — 일시정지가 자동 재생 대기 중에도 듣도록 · 도법 서사 오개념 정정

- **재생 중 일시정지 버튼이 아예 먹지 않던 진짜 원인**: 리모컨 `render()` 가 매 프레임 `bPlay.innerHTML` 을 다시 써서 버튼 안의 SVG 아이콘을 통째로 교체 → mousedown 으로 누른 SVG 가 mouseup 전에 DOM 에서 사라져 브라우저가 click 을 버림(실제 클릭 측정: mousedown 1·mouseup 1·click 0). JS `.click()` 으로는 재현되지 않아 늦게 찾음. 아이콘은 진행 상태가 바뀔 때만 교체하도록 수정. 도법 선택기 버튼 배지(`picker.js sync`)도 같은 패턴이라 도법이 바뀔 때만 다시 그림.
- 리모컨: 한 단계가 끝나고 자동 재생이 다음 단계를 기다리는 1초 동안 버튼이 재생 아이콘이어서, 누르면 멈추지 않고 다음 단계로 점프하던 문제. 대기 중도 "진행 중"(`isRunning`)으로 보아 일시정지 아이콘을 보이고, 누르면 그 자리(t = 1)에서 멈춤. 대기 중 「이전」·스크럽을 눌러도 1초 뒤 단계 끝으로 튀던 것도 함께(대기 취소). 검증 13장 「리모컨 재생」 6항목(105/105).
- Equal Earth: 실제 설계(2018, Šavrič·Patterson·Jenny)는 람베르트를 고친 것도 로빈슨을 고친 것도 아니고, 정적 조건 위에서 로빈슨과 닮은 외형을 목표로 새로 설계한 것임을 조정 ①·② 자막, 수식 설명, 목록 설명에 명시. 계수는 "최적화"가 아니라 시각적 방법으로 정한 값. 가로세로 비 2.05458 : 1(로빈슨 1.97 : 1) 추가.
- 서사 검토(모든 도법): 「조정 ← X」 칩과 도법 선택기의 "빛 투영" 설명에 "이 앱의 구성이지 실제 고안 순서가 아님"을 명시. 로빈슨 ① "로빈슨의 출발점" → "등장방형을 바탕 격자로". 메르카토르 설명의 "메르카토르가 표를 손으로 더함" → 방법 미기록, 정확한 표는 1599년 라이트. 몰바이데 예시 수치 φ = 45° → θ ≈ 36.3°, y ≈ 0.84 로 정정(37.8°, 0.87 은 오류). 심사도법 "60° 밖은 그릴 수 없음" → 반구 경계(90°)가 한계이고 60° 는 이 앱의 표시 범위.

## 2026-09-22 — 간격 그래프 축 글자 겹침 해소

- 측면 패널 간격 함수 그래프(`spacingGraph`)에서 세로축 이름(`y(φ)`/`r(c)`)과 최댓값이 같은 자리에, 가로축 이름(`φ`/`c (접점 각거리)`)이 45°·120°·180° 눈금 위에 겹쳐 찍히던 것을 고침. 마지막 눈금(`90°`/`180°`)이 캔버스 밖으로 잘리던 것도 함께.
- 글자 배치를 `js/ui/graphLayout.js`(순수 함수)로 분리: 윗줄 = 세로축 이름 + 범례, 왼쪽 여백 = 범위 값, 눈금 줄(가운데 정렬·캔버스 안으로 고정), 맨 아랫줄 = 가로축 이름(오른쪽 정렬). 검증 12장 「간격 그래프 글자」 6항목 추가(99/99).

## 2026-09-22 — 헤더 컨트롤 그룹 구분 · 파비콘

- 헤더 컨트롤(축 · 표준위선 · 접선/할선 · 티소 지표 · 비교)을 세로 구분선으로 나눠 ⓘ 버튼이 티소 지표 그룹 것임이 보이게 함. 모바일 서랍에서는 그룹마다 상자.
- 파비콘: 헤더 브랜드 지구본과 같은 도안(남색 바탕 + 노란 선) — `favicon.svg`(기본), `favicon.ico`(16·32·48), `apple-touch-icon.png`(180, iOS 홈 화면), `icon-512.png`. `theme-color` #1B2A4A.

## 2026-09-21 — 모바일 레이아웃: 햄버거 서랍 · 설명 패널 접기/펴기

- 헤더(≤ 860px): 브랜드 아이콘 · 도법 선택 · 햄버거(`#menuBtn`)만. 축·표준위선·티소·비교 컨트롤과 저사양·전체화면·링크 복사는 햄버거 서랍(`#controls` 를 고정 위치 서랍으로)에 세로 배치. 바깥을 누르거나 Esc 로 닫힘.
- 설명 패널: 데스크톱은 뷰포트 오른쪽 위 "패널" 버튼으로 접기/펴기(그리드 열 0 으로, 렌더러 리사이즈), 모바일은 하단 시트 손잡이(도법 이름 + 화살표)로 접기(44px)/펴기(46vh). 접힘 상태는 localStorage 에 기억.
- 리모컨(모바일): 단계 버튼 축소, 자동 재생 라벨 숨김, 유틸 버튼은 서랍으로(같은 상태를 구독하는 두 번째 인스턴스).
- 도법 선택 목록·팝오버는 모바일에서 화면 폭에 맞춤. 리모컨은 모바일 세로에서 단계 줄 + 재생 줄 두 줄.
- 폰 가로 모드(≤ 860px · landscape): 하단 시트 대신 오른쪽 300px 패널 + 접기 버튼, 리모컨 한 줄.
- 햄버거는 헤더 오른쪽 끝, 아이콘 정중앙. iOS 사파리 대응: `100dvh` + `viewport-fit=cover` + 리모컨 `safe-area-inset-bottom` (하단 주소창 밑에 리모컨이 깔리던 문제).
- 문서 제목을 `Projection 3D` 로 단순화. Equal Earth 「쓰이는 곳」에 최근 국제적 채택 내용 추가.

## 2026-09-21 — 도법별 "쓰이는 곳" 블록

- `js/ui/usageNotes.js`: 도법 24개의 사용처(어떤 상황·어떤 나라·기관) — 예: 람베르트 정각원추 = 북반구 중위도 동서로 넓은 나라(미국 항공도·State Plane, 프랑스 Lambert 93, 우리나라 일기도), 횡축 메르카토르 = 우리나라 지형도 TM 좌표계·UTM. 측면 패널의 광원 블록 아래에 초록 핀 아이콘으로 표시. 항상 선택한 도법 기준.

## 2026-09-21 — 빛 단계에 감싼 지구 다시 보이게, 강조색 노랑으로 통일, 자동 재생 기본

- `js/scene/paper.js`: 지구 가림 깊이 패스(`globeOccluder`)는 종이가 불투명할 때만 켠다. 빛 투영 단계(종이 0.5 반투명)에서 원통·원뿔이 감싼 지구와 안쪽 광원이 이전처럼 비쳐 보이도록 복원. 씌우기·펼치기의 불투명 구간에서는 그대로 가림.
- `css/style.css`: 선택 버튼·재생 버튼·현재 단계·선택기 항목의 흰색 강조를 노랑(`--accent`)으로 통일(맞춤 버튼·배지·브랜드와 같은 색).
- `js/state.js`: 자동 재생 기본값 켬.
- 비교(겹쳐 보기) 선택을 네이티브 select 에서 도법 선택기와 같은 드롭다운(`picker.createDropdown`)으로 교체 — 같은 버튼 높이·목록·스크롤바, 성질 배지, 그룹별 정리.
- "프로젝터" 버튼 → **"전체화면"** 토글로 단순화. 전체화면 진입/해제만 하고, 전체화면 상태(`fullscreenchange`)가 곧 프로젝터 스타일(큰 글씨·고대비)로 반영됨. URL 의 `projector=1` 은 스타일만 켜고 전체화면은 사용자 조작이 필요.

## 2026-09-21 — 할선은 교과서에서 할선으로 다루는 도법에만

- 원통: 중심원통·메르카토르·밀러·횡축 메르카토르는 접선 고정(`lockParams: ['phi0']`, 슬라이더 제거). 등장방형만 표준위선 φ₀ 슬라이더 유지(표준위선을 두는 정거원통). 람베르트 정적원통은 이미 고정.
- 원뿔: 람베르트 정각원추·알베르스만 접선/할선 전환(`secant: true`). 중심원추·정거원추는 접선 고정, 표준위선 φ₀ 슬라이더는 유지(90° = 평면, 0° = 원통 연속 변형 시연).
- URL 의 `phi0` 은 잠긴 도법에서 무시.
